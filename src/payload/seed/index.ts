...
import { getStoreCopyright } from '../../constants/brand'
import { seedBulk } from './bulk'
import { cartPage } from './cart-page'
import { DEMO_CATEGORY_MEDIA } from './catalog'
...
export const seed = async (payload: Payload): Promise<void> => {
  ...
  const catalogMedia = await ensureCatalogMedia(payload)

  await Promise.all(
    [apparelCategory, ebooksCategory, coursesCategory].map(category =>
      payload.update({
        collection: 'categories',
        id: category.id,
        data: {
          media: catalogMedia[DEMO_CATEGORY_MEDIA[category.title]],
        },
      }),
    ),
  )
  ...
  if (process.env.SEED_BULK === 'true') {
    ...
    await seedUsers(payload, { customerCount: 10, clearExisting: true })
  }
